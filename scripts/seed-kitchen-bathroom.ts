// scripts/seed-kitchen-bathroom.ts
// Seeds the 3 strong Kitchen & Bathroom packs (hand-crafted, non-generic)
// Run with: SUPABASE_PROJECT_REF="ngwbpuxltyfweikdupoj" npx tsx scripts/seed-kitchen-bathroom.ts
// For live: SUPABASE_PROJECT_REF="ngwbpuxltyfweikdupoj" DRY_RUN=0 npx tsx scripts/seed-kitchen-bathroom.ts

import { kitchenBathroomQuestionPacks } from "../supabase/functions/_shared/kitchenBathroomQuestionPacks.ts";
...
const strongPacks = kitchenBathroomQuestionPacks.filter(p => 
...
console.log(`\n========================================`);
console.log(`Seeding KITCHEN & BATHROOM STRONG PACKS`);
console.log(`========================================\n`);
...
if (res.ok && !DRY_RUN) {
  console.log("\n✅ Kitchen & Bathroom packs seeded successfully!");
  console.log("Run this query to verify:");
  console.log(`
SELECT micro_slug, title, jsonb_array_length(questions) as q_count
FROM question_packs 
WHERE micro_slug IN ('kitchen-fitting', 'bathroom-design', 'wetroom-installation')
  AND is_active = true;
  `);
}
