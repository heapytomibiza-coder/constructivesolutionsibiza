/**
 * Electrical Gold Standard Packs - V1
 * Category Contract: Circuits, loads, protection, compliance
 * Scope units: points, circuits, units (never m²)
 * Evidence: mandatory photos
 * 
 * Micro-slug mapping:
 * - Install Extra Sockets → extra-sockets-power-points
 * - No Power / Tripping Circuits → no-power-tripping-circuits
 * - Electrical Safety Check (EICR) → electrical-safety-checks-reports
 * - Fuse Box / Consumer Unit Replacement → fuse-box-consumer-unit-replacement
 */

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

// Canonical question format for QuestionsStep renderer
...
// Transform user JSON format to canonical format
function transformQuestion(q: {
  ...
  options?: string[];
  helper?: string;
  validation?: { max_length?: number };
  conditional?: { depends_on: string; operator: string; value: string };
  file_types?: string[];
  max_files?: number;
  min_files?: number;
}): QuestionDef {
  ...
  const canonicalType = typeMap[q.type] || "text";
  ...
  if (q.options) {
    result.options = normalizeOptions(q.options);
  }

  if (q.helper) {
    result.help = q.helper;
  }

  if (q.file_types) {
    result.accept = q.file_types.join(",");
  }

  if (q.conditional) {
    result.show_if = {
      questionId: q.conditional.depends_on,
      value: q.conditional.value.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/_+$/, ""),
    };
  }
...
const electricalPacks: QuestionPack[] = [
...
export async function seedElectricalPacks(supabaseUrl: string, supabaseKey: string) {
  const supabase = createClient(supabaseUrl, supabaseKey);

  console.log(`Seeding ${electricalPacks.length} Electrical Gold Standard packs...`);
  ...
  for (const pack of electricalPacks) {
    ...
    if (error) {
      console.error(`Failed to upsert ${pack.micro_slug}:`, error.message);
    } else {
      console.log(`✓ ${pack.micro_slug} (${pack.questions.length} questions)`);
    }
  }
