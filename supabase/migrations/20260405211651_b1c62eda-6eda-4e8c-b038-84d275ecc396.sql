...
DROP TRIGGER IF EXISTS trg_evaluate_pack_rules ON public.jobs;

-- Recreate function with simplified underscore check
...
BEGIN
  ...
  LOOP
    ...
    IF left(v_micro_key, 1) = '_' THEN CONTINUE; END IF;
...
-- Recreate trigger scoped to answers column only
CREATE TRIGGER trg_evaluate_pack_rules
  BEFORE INSERT OR UPDATE OF answers ON public.jobs
  FOR EACH ROW
