...
BEGIN
  ...
  -- CRITICAL: Onboarding bottleneck — >5 users stuck at SAME phase within last 24h (RATE-BASED)
  -- This detects: "are we accumulating new stuck users at a specific step?"
  -- Uses updated_at within 24h AND phase not progressed, meaning these users
  -- entered/stalled at this phase recently.
  ...
  WHERE onboarding_phase NOT IN ('complete', 'not_started')
    ...
    AND updated_at < now() - interval '1 hour'  -- exclude very recent (still active)
  ...
  IF v_phase_count IS NOT NULL AND v_phase_count > 5 THEN
    v_alerts := v_alerts || jsonb_build_object(
      ...
      'body', v_phase_count || ' pro(s) stuck at "' || v_worst_phase || '" in the last 24h — possible systemic issue',
      'count', v_phase_count,
  ...
  -- General stuck onboarding (yellow — snapshot of all currently stuck 6h+)
